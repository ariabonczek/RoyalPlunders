...
public static class InteractionTable
{
    ...
    {
        if (!objA || !objB) // if either object does not exist
            return 0;
        ...
        if (objALevel < 0 || objBLevel < 0) // incomparable (same priority)
            return 0;

        return objBLevel - objALevel; // the difference in priority
    }
}
